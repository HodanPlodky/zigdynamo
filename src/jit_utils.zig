...
pub const JitFunction = struct {
    ...
    pub fn run(self: *const JitFunction, comptime StateType: type, state: *const StateType) void {
        const f: *const fn (*const StateType) callconv(JitCallConv) void = @ptrCast(@alignCast(self.code));
        f(state);
    }
};
...
/// compilers
pub fn JitCompilerBase(comptime StateType: type) type {
    return struct {
        const Self = @This();
        ...
        pub fn init(code_buffer_size: usize, heuristic: Heuristic) Self {
        ...
        pub fn end_compilation(self: *Self) void {
        ...
        pub fn append_offsets(self: *Self, offset: u32, count: usize) void {
            self.offsets.appendNTimesAssumeCapacity(offset, count);
        }

        pub fn append_jump(self: *Self, offset: u32) void {
            self.jumps.appendAssumeCapacity(offset);
        }

        fn fix_jumps(self: *Self, jumps: []const u32, offsets: []const u32) void {
            for (jumps) |jump| {
                // read pc val
        ...
        fn init_panic_handlers(self: *Self) !void {
            try self.emit_panic_handler_with_prolog("binop_panic", Self.bin_op_panic_prolog);
            try self.emit_panic_handler("if_condition_panic");
            try self.emit_panic_handler("string_panic");
        }

        fn bin_op_panic_prolog(self: *Self) !void {
            try self.mov_reg_reg(GPR64.rdi, GPR64.r8);
            try self.mov_reg_reg(GPR64.rsi, GPR64.r9);
        }

        fn emit_panic_handler(self: *Self, comptime panic_name: []const u8) !void {
            @field(self.panic_table, panic_name) = self.code_ptr;
            try self.call(panic_name);
        }

        fn emit_panic_handler_with_prolog(self: *Self, comptime panic_name: []const u8, comptime prolog: fn (*Self) JitError!void) !void {
            @field(self.panic_table, panic_name) = self.code_ptr;
            try prolog(self);
            try self.call(panic_name);
        }
        ...
        pub fn emit_panic(self: *Self, comptime panic_name: []const u8) !void {
        ...
        pub fn emit_break(self: *Self) !void {
            try self.emit_byte(0xcc);
        }

        pub fn emit_slice(self: *Self, inst: []const u8) !void {
        ...
        pub fn emit_byte(self: *Self, byte: u8) !void {
        ...
        pub fn mov_reg_reg(self: *Self, dst: GPR64, src: GPR64) !void {
            // mov rbx, rdi
            // 48 89 fb
            // 48 = REX
        ...
        pub fn set_reg_64(self: *Self, reg: GPR64, value: u64) !void {
        ...
        pub fn deref_ptr(self: *Self, dst: GPR64, src: GPR64) !void {
        ...
        pub fn mov_from_struct_64(self: *Self, dst: GPR64, base: GPR64, offset: u32) !void {
            // REX.W + 8B /r
        ...
        pub fn set_to_index64(self: *Self, scale: Scale, base: GPR64, index: GPR64, offset: u32, src: GPR64) !void {
        ...
        pub fn mov_from_jit_state(self: *Self, dst: GPR64, comptime field_name: []const u8) !void {
            const offset = comptime StateType.get_offset(field_name);
        ...
        pub fn mov_from_jit_state_offset(self: *Self, to_reg: GPR64, offset: u32) !void {
        ...
        pub fn call(self: *Self, comptime function_name: []const u8) !void {
            try self.mov_from_jit_state(GPR64.rax, function_name);
            try self.call_from_rax();
        }

        pub fn call_from_rax(self: *Self) !void {
            // call rax
